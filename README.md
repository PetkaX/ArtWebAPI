...
🧩 Основные сущности
| Сущность | Описание | |--------|--------| | Article | Статья с заголовком, содержанием, датой создания | | Tag | Тег (например, "asp.net", "security") | | Section | Секция (например, "Безопасность"), связана с тегами | | Связь | Article ↔ Tag через ArticleTag, Section ↔ Tag через SectionTag |

Статья попадает в секцию, если у неё есть хотя бы один общий тег.

🌐 API Endpoints
| Метод | Путь | Описание | |------|------|--------| | GET | /api/articles | Получить все статьи | | GET | /api/articles/{id} | Получить статью по ID | | POST | /api/articles | Создать статью | | PUT | /api/articles/{id} | Обновить статью | | DELETE | /api/articles/{id} | Удалить статью | | GET | /api/tags | Получить все теги | | GET | /api/sections | Получить все секции | | GET | /api/sections/{id}/articles | Получить статьи секции |
